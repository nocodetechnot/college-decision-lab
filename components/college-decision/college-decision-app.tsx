...
const initialColleges: College[] = [
  {
    ...
    notes: 'Strong DC policy ecosystem',
  },
  {
    id: uid(),
    name: 'Boston University',
    notes: 'Balanced academics and city access',
  },
  {
    id: uid(),
    name: 'Northeastern University',
    notes: 'Structured co-op path',
  },
  ...
  { id: uid(), name: 'UC Davis', notes: 'Strong research university' },
  {
    ...
    notes: 'Solid traditional option',
  },
  {
    id: uid(),
...
const initialCriteria: Criterion[] = [
  {
    ...
    name: 'Academic Fit',
    description:
      'How well the school matches your academic interests and learning style.',
    weight: 10,
  },
  {
    id: uid(),
    name: 'Economics Strength',
    description: 'Depth, rigor, and quality of economics training.',
    weight: 9,
  },
  {
    id: uid(),
    name: 'International Policy Strength',
    description:
      'Access to policy, global affairs, and international systems coursework.',
    weight: 10,
  },
  {
    id: uid(),
    name: 'Internship Access',
    description: 'Ease of getting meaningful internships during school.',
    weight: 9,
  },
  {
    id: uid(),
    ...
    description:
      'Placement strength for jobs, grad school, and long-term trajectory.',
    weight: 8,
  },
  {
    id: uid(),
    name: 'Cost and Aid',
    description: 'Overall affordability after aid and scholarships.',
    weight: 8,
  },
  {
    id: uid(),
    name: 'Campus and City Fit',
    description:
      'How well the campus and surrounding city fit your preferences.',
    weight: 7,
  },
  {
    id: uid(),
    name: 'Flexibility',
    description: 'Room to change direction if your interests evolve.',
    weight: 7,
  },
];
...
export default function CollegeDecisionWebApp() {
  ...
  const [scores, setScores] = useState<Scores>(() =>
    buildInitialScores(initialColleges, initialCriteria)
  );
  ...
  useEffect(() => {
    ...
    try {
      ...
      setStudentName(parsed.studentName ?? 'FirstName LastName');
      setDecisionGoal(
        parsed.decisionGoal ??
          'Choose the best undergraduate environment for economics, international policy, or a flexible path into future graduate study.'
      );
      ...
      setScores(
        parsed.scores ?? buildInitialScores(initialColleges, initialCriteria)
      );
      setNotes(
        parsed.notes ??
          'Use this tool to compare colleges deliberately. Numbers help structure the choice. They do not replace judgment.'
  ...
  const normalizedWeights = useMemo(() => {
    const total = criteria.reduce(
      (sum, c) => sum + Math.max(0, Number(c.weight) || 0),
      0
    );
    return criteria.map((c) => ({
      ...c,
      normalizedWeight:
        total > 0 ? Math.max(0, Number(c.weight) || 0) / total : 0,
    }));
  }, [criteria]);
  ...
  const results = useMemo(() => {
    const rows = colleges.map((college) => {
      ...
      const completeness = criteria.length
        ? Math.round((completed / criteria.length) * 100)
        : 0;
      return {
        college,
  ...
  const barData = results.map((r) => ({
    name:
      r.college.name.length > 18
        ? `${r.college.name.slice(0, 18)}…`
        : r.college.name,
    score: r.total,
  }));
  ...
  function setScenarioWeights(nextScenario: ScenarioKey) {
    ...
    setCriteria((prev) =>
      prev.map((criterion) => ({
        ...criterion,
        weight: template[criterion.name] ?? criterion.weight,
      }))
    );
  }
  ...
  function updateScore(collegeId: string, criterionId: string, value: string) {
    const cleaned =
      value === ''
        ...
        : Math.max(
            0,
            Math.min(scaleMax, Number.isNaN(Number(value)) ? 0 : Number(value))
          );
  ...
  function addCollege() {
    ...
    setScores((prev) => ({
      ...prev,
      [newCollege.id]: Object.fromEntries(
  ...
  function exportPdf() {
    ...
    autoTable(doc, {
      ...
      body: normalizedWeights.map((c) => [
        c.name,
        String(c.weight),
        c.normalizedWeight.toFixed(3),
        c.description,
      ]),
      styles: { fontSize: 10, cellPadding: 6 },
      headStyles: { fillColor: [30, 41, 59] },
    ...
    doc.save(
      `${studentName
        .toLowerCase()
        .replace(/\s+/g, '-')}-college-decision-report.pdf`
    );
  }
  ...
  return (
      ...
      >
        <div
         ...
         >
          ...
          <SectionCard
            ...
            subtitle="This section explains the model so the user learns while deciding."
          >
          ...
          >
                ...
                >
                  ...
                  >
                    ...
                    <div>
                      ...
                      >
                        {criterion.normalizedWeight.toFixed(3)}
                      </div>
                      <div style={{ marginTop: 10 }}>
            ...
            <div style={{ display: 'grid', gap: 16 }}>
              <div style={{ ...cardStyle, padding: 16 }}>
                ...
                >
                  The raw values express preference. Normalization turns them
                  into proportions that sum to 1.000.
                </div>
                <div
                  style={{
                    marginTop: 16,
                ...
                >
                  ...
                  <div style={{ marginTop: 8 }}>
                    If Academic Fit = 8, Cost = 4, and Internship Access = 8,
                    the total raw weight is 20.
                  </div>
                  <div style={{ marginTop: 8 }}>
                  ...
                  <tr key={college.id}>
                    ...
                    >
                      <input
                        value={college.name}
                        onChange={(e) =>
                          updateCollege(college.id, { name: e.target.value })
                        }
                        style={inputStyle}
                      />
                    </td>
                    ...
                    >
                      <input
                        value={college.notes}
                        onChange={(e) =>
                          updateCollege(college.id, { notes: e.target.value })
                        }
                        style={inputStyle}
                      />
                    </td>
        ...
        >
          ...
          >
            ...
            <SectionCard
              ...
              subtitle="A score is useful when you understand what created it."
            >
              <div
                style={{
                  display: 'grid',
              ...
              >
                <div>
                  ...
                  {topChoice?.college.name || 'N/A'}
                </div>
                <div>
                  <span style={{ fontWeight: 700, color: '#0f172a' }}>
                    Why:
                  </span>{' '}
                  This school currently aligns best with the weighted criteria
                  and scores you entered.
                </div>
                <div>
                  <span style={{ fontWeight: 700, color: '#0f172a' }}>
                    Pedagogical prompt:
                  </span>{' '}
                  Change one major weight by 30 to 50 percent and see whether
                  the winner changes. If the winner stays the same, the decision
                  is more robust.
                </div>
              </div>
            </SectionCard>
          ...
          >
            ...
            <SectionCard
              title="Top 3 Criteria Profile"
              subtitle="Use this to see whether the leader wins broadly or only under a narrow profile."
            >
        ...
        >
          <SectionCard
            title="Report Notes"
            subtitle="These notes will appear in the exported PDF."
          >
            <textarea
              rows={12}
              value={notes}
          ...
          <SectionCard
            ...
            subtitle="Use the app in a disciplined order."
          >
            <div
              style={{
                display: 'grid',
            ...
            >
              {[
                ...
                [
                  'Step 2',
                  'Agree on raw weights before entering school scores.',
                ],
                [
                  'Step 3',
                  'Score each school honestly. Avoid giving every top school a 9 or 10 on every criterion.',
                ],
                [
                  'Step 4',
                  'Review the ranking, then stress test it by changing one major weight at a time.',
                ],
                [
                  'Step 5',
                  'Export the PDF and discuss the result with both numbers and judgment in view.',
                ],
              ].map(([step, body]) => (
                <div
                  key={step}
